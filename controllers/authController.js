...
const resetPassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordResetToken = undefined;
  user.passwordResetExpires = undefined;
  await user.save(); // do not have to turn off validators as we want the inputs to be validated
...
const updatePassword = catchAsync(async (req, res, next) => {
  ...
  // can not use findByIdAndUpdate as validator and other middleware does not work on update
  const user = await User.findById(req.user.id).select('+password'); // must add password as it is select: false by default
